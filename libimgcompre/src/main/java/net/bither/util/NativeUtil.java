...
import com.damo.libimgcompre.CompressConst;
...
public class NativeUtil {
    ...
    public static void compressBitmap(Bitmap bit, String fileName, boolean optimize) {
        saveBitmap(bit, CompressConst.IMAGE_QUALITY, fileName, optimize);
    }
    ...
    public static void compressBitmap(Bitmap bitmap, String filePath) {
        // 获取尺寸压缩倍数
        int ratio = NativeUtil.getRatioSize(bitmap.getWidth(), bitmap.getHeight());
        // 压缩Bitmap到对应尺寸
        ...
        while (baos.toByteArray().length / 1024 > CompressConst.IMAGE_MAXSIZE) {
            // 重置baos即清空baos
            baos.reset();
            // 每次都减少10
            options -= 10;
